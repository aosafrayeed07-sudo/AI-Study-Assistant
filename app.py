...
def generate_ai_quiz(subject, number=10):
    ...
    prompt = f"""
Create exactly {number} multiple-choice questions based ONLY on the selected subject: {subject}.

The selected subject is the ONLY subject the quiz should test.
...
- Every question must be directly relevant to {subject}.
- Do not mix in questions from other subjects unless the selected subject itself requires them.
- Do not substitute a different subject because the selected subject is broad or unfamiliar.
...
def get_question_set(subject_name):
    ...
    # No built-in question bank for this subject.
    # start_quiz() will use Gemini to generate a subject-specific quiz.
    return []
...
def start_quiz():
    ...
    # Every quiz is generated by Gemini from the selected subject.
